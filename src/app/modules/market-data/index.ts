...
import { upsertOhlcv, getLatestOhlcv } from '../../repositories/ohlcv.repository.js';
...
const _buildHistoryUrl = (from: number, to: number): string => {
...
export const syncOhlcv = async (): Promise<void> => {
  const lastSync = await getCache('market:last_sync');
  if (lastSync) {
    return;
  }
  const symbol = ENV.INDODAX_PAIR;
  const timeframeMin = ENV.INDODAX_TIMEFRAME_MIN;
  const latest = await getLatestOhlcv(symbol, timeframeMin, 1);
  const nowSec = Math.floor(Date.now() / 1000);
  const fromSec =
    latest.length > 0
      ? Math.floor(latest[0].time.getTime() / 1000) + timeframeMin * 60
      : nowSec - timeframeMin * 60 * 300;
  const url = _buildHistoryUrl(fromSec, nowSec);
